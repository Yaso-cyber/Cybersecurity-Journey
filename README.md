# 🛡️ Cybersecurity Journey

Tracking my learning path in cybersecurity, documenting milestones, tools, and skills acquired along the way.

---

## 📅 Day 1 – Ubuntu VM Setup
- Installed **Ubuntu VM** in VirtualBox  
- Learned **5 basic Linux commands**  
- Added initial **README** to track progress  

---

## 📅 Day 2 – Linux Practice
- Practiced **`chmod`** and **`nano`** commands  
- Updated progress documentation  

---

## 📅 Day 3 – 21/11/2025

### 🎯 Achievements
- Completed **5 TryHackMe rooms**  
- Earned **1 badge** 🏅  

### 💻 Kali Linux Setup
- Installed **Kali Linux** in VirtualBox  
- Connected to **TryHackMe VPN** successfully  

### 🎨 Terminal Customization
- Enabled **colorful Bash prompt**  
- Configured **colored `ls` output** for easier file navigation  

### 🛠️ Skills Practiced
- Linux command-line basics  
- Virtual machine management  
- VPN connection & troubleshooting  
- Terminal productivity customization  

### 🚀 Next Steps
- Complete more **TryHackMe rooms**  
- Explore **scanning and enumeration tools** (`nmap`, `netcat`)  
- Practice workflow for **connecting to new machines via VPN**  
